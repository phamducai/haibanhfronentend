...
const AdminCourses = () => {
  ...
  const {
    ...
    queryKey: ['products', 'courses', searchQuery],
    queryFn: () => courseService.getCoursesAsProducts(searchQuery || undefined, true),
    enabled: true
  });
  ...
  const handleEditCourse = async (courseId: string) => {
    try {
      ...
      if (courseDetail.imageurl) {
        if (courseDetail.imageurl.startsWith('http')) {
          setImagePreview(courseDetail.imageurl);
        } else {
          setImagePreview(courseDetail.imageurl.startsWith('/') ? courseDetail.imageurl : `/${courseDetail.imageurl}`);
        }
      }
      setShowCourseDialog(true);
    } catch (error) {
  ...
  const handleDeleteCourse = async (courseId: string) => {
    try {
      ...
      toast.success(`Đã xoá khoá học có ID: ${courseId}`);
      
      refetch();
    } catch (error) {
      toast.error('Có lỗi xảy ra khi xoá khoá học.');
    }
  };
  ...
  const onSubmit = async (data: CourseFormValues) => {
    try {
      if (isEditing && currentCourseId) {
        ...
        toast.success(`Đã cập nhật khoá học: ${data.productname}`);
      } else {
        ...
        toast.success(`Đã tạo khoá học mới: ${data.productname}`);
      }
      ...
      toast.error('Có lỗi xảy ra khi lưu khoá học.');
      console.error('Error saving course:', error);
    }
  };
  
  const handleManageChapters = (courseId: string) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">Quản lý khoá học</h2>
                ...
                coursesData.map((course,index) => (
                  <TableRow key={course.productid}>
                    ...
                    <TableCell className="text-right space-x-2">
                      <Button variant="outline" size="sm" onClick={() => handleManageChapters(course.productid)}>
      ...
      <Dialog open={showCourseDialog} onOpenChange={setShowCourseDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{isEditing ? 'Chỉnh sửa khoá học' : 'Thêm khoá học mới'}</DialogTitle>
          </DialogHeader>
        ...
        Object.entries(errors).forEach(([field, error]) => {
          toast.error(`Lỗi: ${error?.message || 'Vui lòng kiểm tra lại trường ' + field}`);
              ...
              <DialogFooter>
                ...
                <Button type="submit">
                  {isEditing ? 'Cập nhật' : 'Tạo khoá học'}
                </Button>
              </DialogFooter>
            </form>
          </Form>
