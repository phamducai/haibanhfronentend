...
import { BASE_URL_LOCAL } from "../endpoints";
...
export const userProductService = {
  async createUserProduct(
    userProductData: CreateUserProductService
  ): Promise<boolean> {
    try {
  ...
  async getUserProductsByUserId(
    status?: boolean
  ): Promise<GetUserProductService[]> {
    try {
      const response = await axios.get<GetUserProductService[]>(
        `${BASE_URL_LOCAL}/userproducts/userid/id`,
        {
          params: { status },
        }
      );
  ...
  async updateUsersProducts(
    id: string,
    updateProductData: UpdateProcuctService
  ): Promise<boolean> {
    try {
